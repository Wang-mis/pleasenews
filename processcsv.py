...
def mergeMentions(mention_path="./mentions/", export_path="./export/", merge_save="./merge/", filter=["RUS", "UKR"]):
    ...
    for ele_dir in mention_dir:
        ...
        for csv in os.listdir(ele_dir_path):
            if csv.split(".")[-1].lower() != 'csv':  # 如果当前文件不是CSV文件，直接跳过
                print(csv)
                continue
            sub_dfs.append(pd.read_csv(ele_dir_path + csv, delimiter='\t'))  # 读取mentions.CSV

        result = pd.concat(sub_dfs)  # 合并一天中的所有mentions.CSV

        process_df = pd.read_csv(export_path + ele_dir + ".export.CSV", delimiter='\t')  # 读取export.CSV
...
def merge_medialist():
    TMPATH = "./merge/NULL/" + str(TIME_RANGE[0]) + '_' + str(TIME_RANGE[1])
    df = pd.read_csv(TMPATH + ".merge.csv")
    df = df[df["MentionSourceName"].isin(MEDIUMLIST)]  # 筛选在MEDIUMLIST列表里的媒体发布的文章
